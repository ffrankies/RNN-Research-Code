...
# GRU units and using the Adam algorithm in place of simple stochastic gradient
# descent, to predict the next word in a sentence. The nature of this
...
class AdamRNN(object):
    """
    A Recurrent Neural Network with GRU units using the Adam algorithm in place
    of the simpler stochastic gradient descent. The Adam algorithm should
    speed up the training of this network as well as make it slighlty more
    precise. The network essentially looks like this, with the hidden units
    being GRU units:
    ...
    def __init__(self, voc_size=8000, hid_size=100, trunc=4, model=None):
        ...
        if model is None:
            ...
            self.m_w_ih = theano.shared(
                name='m_w_ih',
                value=np.zeros((3, hid_size, voc_size)).astype(
                      theano.config.floatX))
            self.v_w_ih = theano.shared(
                name='v_w_ih',
                value=np.zeros((3, hid_size, voc_size)).astype(
                      theano.config.floatX))
            ...
            self.m_w_hh = theano.shared(
                name='m_w_hh',
                value=np.zeros((3, hid_size, hid_size)).astype(
                      theano.config.floatX))
            self.v_w_hh = theano.shared(
                name='v_w_hh',
                value=np.zeros((3, hid_size, hid_size)).astype(
                      theano.config.floatX))
            ...
            self.m_w_ho = theano.shared(
                name='m_w_ho',
                value=np.zeros((voc_size, hid_size)).astype(
                      theano.config.floatX))
            self.v_w_ho = theano.shared(
                name='v_w_ho',
                value=np.zeros((voc_size, hid_size)).astype(
                      theano.config.floatX))
            ...
            self.m_bias = theano.shared(
                name='m_bias',
                value=np.zeros((3, hid_size)).astype(theano.config.floatX))
            ...
            self.m_out_bias = theano.shared(
                name='m_out_bias',
                value=np.zeros((voc_size)).astype(
                      theano.config.floatX))
            self.v_out_bias = theano.shared(
                name='v_out_bias',
                value=np.zeros((voc_size)).astype(
                      theano.config.floatX))
            ...
            with open(model, "rb") as modelFile:
                ...
                self.v_w_ih = params[12]
                self.m_w_hh = params[13]
                self.v_w_hh = params[14]
                self.m_w_ho = params[15]
                self.v_w_ho = params[16]
                self.m_bias = params[17]
                self.v_bias = params[18]
                self.m_out_bias = params[19]
                self.v_out_bias = params[20]
                self.decay = params[21]
                self.eps = params[22]
        # End of if statement
        ...
        def adamUpdates(loss, parameters):
            """
            Source: https://gist.github.com/skaae/2fcd1c6d6d30f5fe9243
            Returns the updates necessary for the Adam gradient descent
            algorithm. Previous implementation, not using said source,
            produced NaNs, because of 'Big' numbers in the v tensors.
            This method is modified from source to fit my intuition, this
            particular model, and the original research paper.

            :type loss: float32 or T.tensorVariable of type float32.
            :param loss: the current calculated loss of the network.

            :type parameters: a list of T.tensor() symbolic variables.
            :param parameters: all the parameters of the network to which
                               the Adam algorithm will be applied.
            """
            updates = []
            gradients = T.grad(loss, parameters)

            for param, grad in zip(parameters, gradients):
                # Initialize the two momentum vectors for each parameter.
                # If used correctly, this will only be called once.
                m = theano.shared(
                    np.zeros(param.get_value().shape).astype(
                    theano.config.floatX))
                v = theano.shared(
                    np.zeros(param.get_value().shape).astype(
                    theano.config.floatX))

                m_t = self.decay[0] * m + (1. - self.decay[0]) * grad
                v_t = self.decay[1] * v + (1. - self.decay[1]) * T.sqr(grad)

                # m_t_update = m_t / ((1. - self.decay[0]) ** self.time_step)
                # v_t_update = v_t / ((1. - self.decay[1]) ** self.time_step)

                temp = m_t / (T.sqrt(v_t) + self.eps)
                param_update = param - (self.learning_rate * temp)
        ...
        # Time_step parameter used for correction of Adam parameters to
        # prevent bias towards 0.
        self.time_step = 1

        # Adam updates

        # new_m_w_ih = (self.decay[0] * self.m_w_ih + (1 - self.decay[0]) *
        #              d_wght_ih) / ((1 - self.decay[0]) ** self.time_step)
        # new_v_w_ih = (self.decay[1] * self.v_w_ih + (1 - self.decay[1]) *
        #              T.sqr(d_wght_ih)) / ((1 - self.decay[1]) ** self.time_step)
        #
        # new_m_w_hh = (self.decay[0] * self.m_w_hh + (1 - self.decay[0]) *
        #              d_wght_hh) / ((1 - self.decay[0]) ** self.time_step)
        # new_v_w_hh = (self.decay[1] * self.v_w_hh + (1 - self.decay[1]) *
        #              T.sqr(d_wght_hh)) / ((1 - self.decay[1]) ** self.time_step)
        #
        # new_m_w_ho = (self.decay[0] * self.m_w_ho + (1 - self.decay[0]) *
        #              d_wght_ho) / ((1 - self.decay[0]) ** self.time_step)
        # new_v_w_ho = (self.decay[1] * self.v_w_ho + (1 - self.decay[1]) *
        #              T.sqr(d_wght_ho)) / ((1 - self.decay[1]) ** self.time_step)
        #
        #
        # new_m_bias = (self.decay[0] * self.m_bias + (1 - self.decay[0]) *
        #              d_bias) / ((1 - self.decay[0]) ** self.time_step)
        # new_v_bias = (self.decay[1] * self.v_bias + (1 - self.decay[1]) *
        #              T.sqr(d_bias)) / ((1 - self.decay[1]) ** self.time_step)
        #
        # new_m_out_bias = (self.decay[0] * self.m_out_bias + (1 - self.decay[0])
        #                  * d_out_bias) / ((1 - self.decay[0]) ** self.time_step)
        # new_v_out_bias = (self.decay[1] * self.v_out_bias + (1 - self.decay[1])
        #                  * T.sqr(d_out_bias)) / ((1 - self.decay[1]) **
        #                  self.time_step)
        #
        # # The list of updates needed for Adam SGD to work
        # adam_updates = [
        #     (self.m_w_ih, new_m_w_ih),
        #     (self.v_w_ih, new_v_w_ih),
        #     (self.m_w_hh, new_m_w_hh),
        #     (self.v_w_hh, new_v_w_hh),
        #     (self.m_w_ho, new_m_w_ho),
        #     (self.v_w_ho, new_v_w_ho),
        #     (self.m_bias, new_m_bias),
        #     (self.v_bias, new_v_bias),
        #     (self.m_out_bias, new_m_out_bias),
        #     (self.v_out_bias, new_v_out_bias),
        #     (self.wght_ih, self.wght_ih - self.learning_rate *
        #      self.m_w_ih / (T.sqrt(self.v_w_ih) + self.eps)),
        #     (self.wght_hh, self.wght_hh - self.learning_rate *
        #      self.m_w_hh / (T.sqrt(self.v_w_hh) + self.eps)),
        #     (self.wght_ho, self.wght_ho - self.learning_rate *
        #      self.m_w_ho / (T.sqrt(self.v_w_ho) + self.eps)),
        #     (self.bias, self.bias - self.learning_rate *
        #      self.m_bias / (T.sqrt(self.v_bias) + self.eps)),
        #     (self.out_bias, self.out_bias - self.learning_rate *
        #      self.m_out_bias / (T.sqrt(self.v_out_bias) + self.eps))
        # ]

        adam_updates = adamUpdates(out_error, [self.wght_ih, self.wght_hh,
                       self.wght_ho, self.bias, self.out_bias])
    ...
    def train_rnn(self, learning_rate=0.005, epochs=1, patience=10000,
        ...
        if max is None:
            ...
            loss = self.calculate_loss(self.x_train[:max], self.y_train[:max])
        ...
        for e in range(epochs):
            ...
            if max is None:
                ...
                for example in range(len(self.y_train[:max])):
                    #try:
                    self.sgd_step(
                        self.x_train[example],
                        self.y_train[example]
                    )
                    #
                    # End of try-catch
            ...
            if (len(losses) > 1 and losses[-1][1] > losses[-2][1]):
                ...
                self.log.info("Setting learning rate to %f" %
                              self.learning_rate)
            ...
            if testing == False:
                params = (
                    ...
                    self.bias, self.out_bias, self.m_w_ih, self.v_w_ih,
                    self.m_w_hh, self.v_w_hh, self.m_w_ho, self.v_w_ho,
                    self.m_bias, self.v_bias, self.m_out_bias, self.v_out_bias,
                    self.decay, self.eps
                )
