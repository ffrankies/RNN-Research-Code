...
class VanillaRNN(object):
    ...
    def __init__(self, voc_size=8000, hid_size=100, trunc=4, model=None):
        ...
        self.log = logging.getLogger("TEST.Vanilla")
    ...
    def train_rnn(self, learning_rate=0.005, epochs=1, patience=10000,
        ...
        for epoch in range(epochs):
            ...
            if testing == False:
                ...
                if path == None:
                    with open("models/reladred" + str(epoch) + ".pkl", "wb") as file:
...
if __name__ == "__main__":
    ...
    arg_parse.add_argument("-m", "--max", type=int,
    ...
    if args.dir == None:
        sentenceDir = "./Vanilla/sentences/" + time.strftime("%d%m%y%H")
        modelDir = "./Vanilla/models/" + time.strftime("%d%m%y%H")
        logDir = "./Vanilla/logs/" + time.strftime("%d%m%y%H")
    else:
        sentenceDir = args.dir + "/sentences/" + time.strftime("%d%m%y%H")
        modelDir = args.dir + "/models/" + time.strftime("%d%m%y%H")
        logDir = args.dir + "/logs/" + time.strftime("%d%m%y%H")
    ...
    RNN.train_rnn(
        epochs=args.epochs,
        patience=100000 if args.max == None else args.max,
