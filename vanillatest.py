...
if args.dir == None:
    sentenceDir = "./Vanilla/" + time.strftime("%d%m%y%H") + "/sentences/"
    modelDir = "./Vanilla/" + time.strftime("%d%m%y%H") + "/models/"
    logDir = "./Vanilla/" + time.strftime("%d%m%y%H") + "/logs"
else:
...
model = RNN.VanillaRNN(voc_size=args.voc_size, hid_size=50, trunc=1000)
