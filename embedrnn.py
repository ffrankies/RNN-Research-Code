...
class EmbedRNN(object):
    """
    A Recurrent Neural Network that uses GRU units (neurons) in a hidden
    layer for combating vanishing and exploding gradients, as well as a word
    embedding layer for capturing meaning from words. The GRU is similar
    to the LSTM, but involves less computations, and is therefore more
    efficient. The network essentially looks like this, with the hidden units
    being GRU units:
    ...
    def __init__(self, voc_size=8000, embed_size=100, hid_size=100, trunc=4,
        ...
        :type embed_size: int
        :param embed_size: the number of dimensions that the input word is
                           represented as in the embedding layer.
    ...
    def train_rnn(self, learning_rate=0.005, epochs=1, patience=10000,
        ...
        for e in range(epochs):
            ...
            if testing == False:
                params = (
                    self.vocabulary_size, self.embed_size, self.hidden_size,
                    self.bptt_truncate, self.weights_ie, self.weights_eh,
                    self.weights_hh, self.weights_ho, self.vocabulary,
                    self.index_to_word, self.word_to_index, self.bias,
                    self.out_bias, self.cache_ie, self.cache_eh, self.cache_hh,
                    self.cache_ho, self.cache_bias, self.cache_out_bias
                )

                if path == None:
                    modelPath = "models/reladred" + str(epoch) + ".pkl"
                    self.createDir("models/")
                    with open(modelPath, "wb") as file:
                        cPickle.dump(params, file, protocol=2)


                    embedPath = "embeddings/reladredsents.pkl"
                    self.createDir("embeddings/")
                    with open(embedPath, "wb") as efile:
                        cPickle.dump(self.weights_ie, efile, protocol=2)

                else:
                    self.createDir("embeddings/")
                    with open("embeddings/reladredsents.pkl") as efile:
                        cPickle.dump(self.weights_ie, efile, protocol=2)
    ...
    def grad_check(self):
        ...
        model_parameters = ['weights_ie', 'weights_eh', 'weights_hh',
                            'weights_ho', 'bias', 'out_bias']
...
if __name__ == "__main__":
    ...
    arg_parse.add_argument("-d", "--dir", default="./embedrnn",
                           help="Directory for storing logs.")
    arg_parse.add_argument("-f", "--filename", default="embedrnn.log",
    ...
    testlog.info("Initializing a new EmbedRNN with logging")

    RNN = EmbedRNN(model=args.model)
    RNN.load_data()
