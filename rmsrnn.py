...
class RMSpropRNN(object):
    ...
    def grad_check(self, max_ex):
        try:
            ...
            print("Found a big number/an infinity")
            my_params = [self.cache_ih, self.cache_hh, self.cache_ho,
                         self.cache_bias, self.cache_out_bias]
            for p in my_params:
                maximum = np.max(p.get_value())
                minimum = np.min(p.get_value())
                print("%s: max %f min %f" % (p.name, maximum, minimum))

            my_grads = [self.d_weights_ih, self.d_weights_hh, self.d_weights_ho,
                        self.d_bias, self.d_out_bias]
            for g in my_grads:
                maximum = np.max(g.get_value())
                minimum = np.min(g.get_value())
                print("%s: max %f min %f" % (g.name, maximum, minimum))

            print("params after cache updates")
            for i in range(5):
                my_params[i] = (0.9 * my_params[i]) + (0.1 * T.sqr(my_grads[i]))
                maximum = np.max(my_params[i].eval())
                minimum = np.min(my_params[i].eval())
                print("%s: max %f min %f" % (my_params[i].name, maximum, minimum))

            update_ih = self.weights_ih - 0.05 * self.d_weights_ih / (T.sqrt(self.cache_ih + 1e-6))
            update_hh = self.weights_hh - 0.05 * self.d_weights_hh / (T.sqrt(self.cache_hh + 1e-6))
            update_ho = self.weights_ho - 0.05 * self.d_weights_ho / (T.sqrt(self.cache_ho + 1e-6))
            update_bias = self.bias - 0.05 * self.d_bias / (T.sqrt(self.cache_bias + 1e-6))
            update_out_bias = self.out_bias - 0.05 * self.d_out_bias / (T.sqrt(self.cache_out_bias + 1e-6))

            updates = [update_ih, update_hh, update_ho, update_bias,
                       update_out_bias]

            for i in range(5):
                maximum = np.max(updates[i].eval())
                minimum = np.min(updates[i].eval())
                print("update %s: max %f min %f" % (my_params[i].name, maximum, minimum))

            self.calculate_loss(self.x_train[:max_ex], self.y_train[:max_ex])
    # End of grad_check()
        ...
        for pidx, pname in enumerate(model_parameters):
            ...
            while not it.finished:
                ...
                gradminus = self.calculate_total_loss(
                    [self.x_train[0]],[self.y_train[0]])
                print("gradplus %f, gradminus %f" % (gradplus, gradminus))
                estimated_gradient = (gradplus - gradminus)/(2*h)
                parameter[ix] = original_value
                parameter_T.set_value(parameter)
                # The gradient for this parameter calculated using backpropagation
                backprop_gradient = bptt_gradients[pidx][ix[0]][ix[1]][ix[2]]
